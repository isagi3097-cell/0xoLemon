# 0xoLemonCore — Feature Reference

This document describes every subsystem in 0xoLemonCore, its purpose, the Steam internals it touches, and the configuration interface exposed to SteaMidra via Lua scripts.

---

## Injection chain

Steam loads DLLs from its own directory on startup.  0xoLemonCore exploits this by placing two thin proxy DLLs alongside `steam.exe`:

- `dwmapi.dll` — forwards the full DWM API surface and loads `0xoCore.dll` on attach
- `xinput1_4.dll` — forwards XInput 1.4 exports; acts as a backup load gate, calling `LoadLibraryA("0xoCore.dll")` on process attach as well
When Steam starts, Windows loads the proxy DLLs before any game code runs.  The proxy's `DllMain` loads `0xoCore.dll` and returns.

Everything below runs inside `steam.exe` itself, so it behaves identically whether Steam was started by the launcher, a Start-menu shortcut, a tray restart, or a scheduled task. No launcher process is required after the DLLs are in place.
...
The schema matches the runtime pattern map format — TOML files dropped into `<Steam>\0xolemoncore\pattern\` resolve without further conversion.

### Source priority

For each DLL, the fetcher tries sources in this order:

1. **User mirror** (optional). If `[pattern_fetch] mirror` is set in `0xolemoncore.toml`, the fetcher substitutes `{subdir}` (`steamclient` or `steamui`) and `{sha}` into the URL and treats it as the first try. Any failure (HTTP 4xx/5xx, network error, parse error) logs a debug line and falls through.
...
4. **Local cache** — `<Steam>\0xolemoncore\pattern\<sha>.toml`. Always written-through on a successful fetch and always read on a network miss.
...
If Steam updates and 0xoLemonCore stops resolving hooks for the new client, downgrade Steam if possible and report the Steam update to the maintainer with the collected 0xoLemonCore logs.

The runtime fetcher's own logs (`<Steam>\0xolemoncore\misc.log`) note every overlay, cache, and network step so failed pattern resolution can be triaged from user logs.
...
Steam uses an internal IPC bus to route messages between its client service and the UI process.  The hook intercepts `IPCProcessMessage`, inspects the command code, and dispatches it to any registered 0xoLemonCore handlers.  Currently the following handlers are active:
...
When the Lua config calls `setEticket()` or `seteticketurl()`, the fetcher issues an HTTP request to the configured URL and writes the returned blob into 0xoLemonCore's credential store. The eticket then feeds into the AppTicket forge pipeline for Denuvo-protected games that need a valid encrypted ticket to pass the DRM check.

---

### OnlineFixInject (`hooks/client/OnlineFixInject.cpp`)

Detours `CreateProcessW` and `CreateProcessAsUserW` to inject `0xoPayload.dll` into game processes launched through the 480 route.

When Steam spawns a manual `-onlinefix` game process, the CreateProcess hook claims the queued executable, creates the process suspended, loads the payload DLL, and resumes the thread. 0xoLemonCorePayload then handles EOS bridge / lobby redirection for online-fix multiplayer. Steam Stub auto launches do not use this payload path.
...
The first built-in provider is fetched with its required compatibility User-Agent internally. Custom URLs and the other fallback providers keep 0xoLemonCore's normal runtime HTTP User-Agent.

---

### Manifest auto-update (`config/LuaLoader` + `runtime/HubcapManifestSync.cpp`)

Manifest gid ownership is decided at Lua parse time, and it is the whole difference
between a depot that follows the latest build and one that is deliberately held:

| Lua | Meaning |
|---|---|
| `setManifestid(depotId, gid)` in an active file | Pin: the configured gid wins |
| `skipManifestPin(depotId)` | Auto-update: the depot follows the public gid |
| `addappid(depotId)` only | Auto-update: resolved from the metadata mirror when Steam reports gid 0 |

`Bind_skipManifestPin` clears any pin parsed earlier in the same file, so the last
statement in a file decides. `ManifestBind::SlapManifestOverrides` applies the result
to Steam's dependency vector on every rebuild; when a pin names a gid the provider no
longer serves, a pin-latest check re-points it at the current public gid instead of
leaving the install stuck on a build Hubcap has retired.

`runtime/HubcapManifestSync.cpp` performs the network side of this. Two properties
matter for reliability, because none of this runs while the launcher is open:

- **Nothing here needs the launcher.** API keys are resolved from the core's own
  settings, `HUBCAP_API_KEY` / `MANIFESTHUB_API_KEY`, a key file in the Steam folder,
  or the launcher's settings JSON when it happens to exist. A launch with the
  launcher closed still resolves its key and still heals from the local vault.
- **Background passes are quota-free.** The periodic worker only auto-heals from the
  local vault and never spends a provider download; Hubcap API downloads stay strictly
  on-demand, triggered when Steam actually asks for a manifest during an install.

Manifest state (request codes, permanent 401/403 gids, definitive not_found) is
persisted by `runtime/ManifestStateCache.cpp`, so a Steam restart does not re-ask for
answers already known.

---

### Session guardrails (`runtime/ProcessInspect.cpp` + `runtime/IntegrityScanner.cpp`)

Every one of the hooks above is meant to survive Steam being launched directly from
the Start menu, a shortcut, the tray restart, or any launcher that is not this one.
Two rules keep that safe:

- **Refuse the wrong process.** The proxy DLLs only load `0xoCore.dll` when the host
  module is `steam.exe` (case-insensitive). A game that imports the same proxy name, or
  a `steamwebhelper` child, falls through to the system implementation untouched.
- **Refuse the wrong state.** `IntegrityScanner` and `ProcessInspect` run before hook
  install, so an unexpected Steam build is reported through `status.json` and the
  launcher status line instead of being patched blind.
...
- `Player.GetUserStats` and `ClientGetUserStats` — rewrite stats requests for Lua stats roots so achievements can load from 0xoLemonCore's stats SteamID pool.
...
When no SteamID override is provided, 0xoLemonCore tries the built-in stats SteamID pool and remembers the first response that returns useful schema, stat, or achievement data.
...
0xoLemonCore does not redirect Steam Cloud files or touch save folders on disk. If a user already has saves split between account folders like `0` and their Steam account ID, back up both folders before launching the game again and move the wanted save manually.
...
### SteamlessApply + SteamlessWatch (`runtime/SteamlessApply.cpp`, `runtime/SteamlessWatch.cpp`)

Native, dependency-free SteamStub remover plus the watcher that drives it.

- **SteamlessApply** drives the upstream Steamless CLI that the launcher ships under `resources/gse-uc/embedded/steamless` (`Steamless.CLI.exe` + its seven unpacker plugins). It does **not** re-implement SteamStub removal: a hand-written variant detector only ever covers a few byte patterns, while upstream handles Variant 1.0/2.0/2.1 x86, 3.0 x86/x64 and 3.1 x86/x64. `SetComponentDir()` is called once at bootstrap from `Settings::ResolveSteamlessComponentDir()` (explicit `[steamless] component_dir`, then next to `0xoCore.dll`, then the launcher resource tree). `Unpack()` spawns the CLI with stdout/stderr piped, reads the `File Saved As:` line, keeps the pristine exe as `<filename>.bak` (so `re4.exe` → `re4.exe.bak`) and writes the unpacked image over the original. `DetectProtected()` is a header-only read (MZ/PE + section table) used to decide whether a game needs fixing — deliberately size-independent, because AAA shipping exes exceed any whole-file scan ceiling.
- **SteamlessWatch** polls `<library>/steamapps/appmanifest_<appid>.acf` for every Lua-tracked app and acts on the **downloading → installed** edge (the `StateFlags` losing every UpdateRunning/Downloading/Staging/Committing/… bit and gaining FullyInstalled). It only touches apps the Lua layer tracks and Steam does not own, and skips a game that already has a `.bak` backup or no `.bind` section. It also runs one sweep right after a `.lua` is dropped (driven from `DirWatch`), and `OnGameLaunch()` is called from the `CUser_SpawnProcess` hook as a fallback the moment Play is pressed in Steam — covering games installed before the core loaded or exes restored by hand. Gated by `[steamstub] auto_steamless`.
...
SteaMidra writes `.lua` files to `Steam\config\stplug-in\<appid>.lua`.  0xoLemonCore watches this directory and reloads files as they change.
...
Pins the manifest GID for depot 1001. 0xoLemonCore reports this GID when Steam asks for the active manifest.
...
### SteamStub removal (Steamless)

```lua
steamlessApply(1234567)    -- strip SteamStub from the game's main exe on disk
steamlessRestore(1234567)  -- put the pristine .exe back from the .bak backup
```

`steamlessApply` runs the launcher-shipped `Steamless.CLI.exe`
(`runtime/SteamlessApply.cpp`) against the game's main executable; it does not
re-implement SteamStub removal. The CLI writes `<name>.unpacked.exe` next to the
input, which is then copied over the original, with the pristine file kept as
`<filename>.bak` (e.g. `re4.exe.bak`). Because the work happens in upstream
Steamless, every variant its plugins support is covered — Variant 1.0/2.0/2.1
x86, 3.0 x86/x64 and 3.1 x86/x64. Both functions are **blocking** (the CLI is
waited on, with a 10-minute cap) and return a short status string. Scripts call
them from the DirWatch worker, never from a Steam UI thread.

Steamless runs automatically when `[steamstub] auto_steamless` is enabled; the
Lua bindings are for on-demand use from a shop plugin.

---

## Configuration file (`0xolemoncore.toml`)

Placed in the Steam installation directory.  SteaMidra writes this file during 0xoLemonCore setup.
...
All other settings use built-in defaults. The SteamStub controls live under:

```toml
[steamstub]
# Spacewar (480) routing for SteamStub games without a manual -onlinefix flag.
auto_enabled = false
# Strip SteamStub from a Lua-tracked game's exe *on disk* the moment Steam
# reports the download finished. Dependency-free: the remover is compiled into
# 0xoCore.dll, so no .NET / Steamless.CLI.exe is needed. The original is kept
# as <filename>.bak (undo with steamlessRestore(appId)).
auto_steamless = false
# Poll interval for the download-watcher, in seconds (clamped to 5..600).
auto_steamless_interval_sec = 15
[steamless]
# Directory holding Steamless.CLI.exe plus its unpacker plugins. Leave empty to
# let the core resolve it (next to 0xoCore.dll, then the launcher resource
# tree at resources/gse-uc/embedded/steamless).
component_dir = ""
```

## Logging

Logging is compiled in only for Debug builds (`0XOLEMONCORE_LOGGING_ENABLED` define).  Release builds compile all `LOG_*` macros to no-ops so there is no runtime overhead.

When enabled, logs are written to `Steam\0xolemoncore\` alongside `0xoCore.dll`.  Each module writes to its own file:
...
Log level is controlled by `0xolemoncore.toml` under `[log] level = "debug"` (default: `info`).
