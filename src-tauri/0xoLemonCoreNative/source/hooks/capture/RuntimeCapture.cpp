        ...
        if (pExInfo->ExceptionRecord->ExceptionCode == EXCEPTION_BREAKPOINT) {
            ...
            if (g_spawnProcessTarget
                ...
                *g_spawnProcessTarget = 0x48;
                ...
                // Steamless is the on-disk first choice. It must finish before
                // we inspect/route the launch, otherwise a successful unpack
                // can still be followed by the SteamStub runtime route.
                SteamlessWatch::LaunchFixResult steamlessResult =
                    SteamlessWatch::LaunchFixResult::NotEligible;
                if (hasDepot && !owned && exePath) {
                    steamlessResult = SteamlessWatch::OnGameLaunch(appId, exePath);
                }
                const bool steamlessClean =
                    steamlessResult == SteamlessWatch::LaunchFixResult::Applied
                    || steamlessResult == SteamlessWatch::LaunchFixResult::AlreadyClean;

                // RUNE is strictly the second, on-disk fallback. It is never
                // deployed when Steamless succeeded or the executable was
                // already clean. Deploy only after a real Steamless failure.
                if (steamlessResult == SteamlessWatch::LaunchFixResult::Failed
                    && exePath) {
                    RuneSteamStub::Deploy(std::filesystem::u8path(exePath));
                }

                // A known-list/probe route is only eligible when Steamless did
                // not establish a clean executable. This enforces the strict
                // priority: Steamless first, runtime fallback second.
                bool knownSteamStub = !steamlessClean
                    && Ticket::IsKnownSteamDrmApp(appId);
                ProtectionProbe::ScanResult steamStubProbe{};
                if (hasDepot && !owned && !steamlessClean && !knownSteamStub && exePath) {
                ...
                const bool detectedSteamStub = !steamlessClean && (knownSteamStub || probeSteamStub);
                ...
                // OnGameLaunch above is deliberately synchronous. Do not call
                // it a second time here: the result already controls whether
                // the runtime SteamStub fallback is eligible.
