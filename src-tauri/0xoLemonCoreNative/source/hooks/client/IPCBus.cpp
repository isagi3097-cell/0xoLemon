...
namespace {
    ...
    // AppId member offset inside the IClientRemoteStorage object is resolved at
    // startup by StructOffsets so a Steam layout shift is absorbed by publishing
    // a new value rather than editing this hook.
    LM_HOOK(IClientRemoteStorage_FileExists, bool, void* pThis, const char* pchFile)
    {
        uint32_t* pAppId = reinterpret_cast<uint32_t*>(
            reinterpret_cast<uint8_t*>(pThis) + StructOffsets::RemoteStorageAppIdOffset());
