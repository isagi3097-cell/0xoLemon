...
namespace {
    ...
    void Post_RequestEncryptedAppTicket(CSteamPipeClient* pipe, CUtlBuffer* pRead, CUtlBuffer* pWrite) {
        ...
        // The payload is a 4-byte length followed by nonceLen bytes, so validate both
        // the length field and the complete payload before reading either one.
        constexpr int32 kArgsOffset = 10;
        constexpr int32 kNonceLengthSize = static_cast<int32>(sizeof(uint32_t));
        if (pRead->m_Put >= kArgsOffset + kNonceLengthSize) {
            const uint8_t* args = pRead->m_Memory.m_pMemory + kArgsOffset;
            uint32_t nonceLen = 0;
            memcpy(&nonceLen, args, sizeof(nonceLen));
            if (nonceLen > 0 && nonceLen < 1024 &&
                pRead->m_Put >= kArgsOffset + kNonceLengthSize + static_cast<int32>(nonceLen)) {
                std::span<const uint8_t> nonce(args + kNonceLengthSize, nonceLen);
