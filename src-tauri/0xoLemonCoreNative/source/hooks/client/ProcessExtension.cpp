...
namespace {
    ...
    std::optional<std::filesystem::path> ResolveConfiguredLibrary(const std::string& configured) {
        std::filesystem::path steamRoot(Utf8ToWide(SteamInstallPath));
        return InjectionPolicy::CanonicalDllPath(std::filesystem::path(Utf8ToWide(configured)), steamRoot);
    }

    InjectionPolicy::Architecture ToPolicyArchitecture(RemoteTools::ProcessBits bits) {
        if (bits == RemoteTools::ProcessBits::X86) return InjectionPolicy::Architecture::X86;
        if (bits == RemoteTools::ProcessBits::X64) return InjectionPolicy::Architecture::X64;
        return InjectionPolicy::Architecture::Unknown;
    }
...
namespace ProcessExtension {
    ...
    void OnGamePipe(const PipeWatch::ProcessSnapshot& snapshot) {
        ...
        std::string library;
        const std::string image = snapshot.imageName.empty() ? snapshot.imagePath : snapshot.imageName;
        const auto matches = InjectionPolicy::OrderedMatches(Settings::processExtensionRules, image, effectiveAppId);
        for (const auto* rule : matches) {
            const auto& candidate = InjectionPolicy::LibraryFor(*rule, ToPolicyArchitecture(bits));
            if (!candidate.empty()) {
                library = candidate;
                break;
            }
        }
        // Backward compatibility: the legacy x86/x64 pair remains the fallback.
        if (library.empty()) library = SelectConfiguredLibrary(bits);
        if (library.empty()) {
        ...
        auto resolved = ResolveConfiguredLibrary(library);
        if (!resolved) {
            LOG_IPCCH_WARN("ProcessExtension: rejected missing or unsafe helper for pid={} appid={} configured={}",
                           snapshot.key.pid, effectiveAppId, library);
            return;
        }
        const std::filesystem::path& helperPath = *resolved;
