...
namespace {
    ...
    //   +0xAB8 from top-manager -> CSteamUIAppController*   (StructOffsets)
    //   +1744  m_vecAppOverviewChanged (subscriber vec data ptr)
    //   +1760  m_vecAppOverviewChanged size
    //
    // The controller offset is resolved at startup by StructOffsets (a Valve
    // shift is absorbed by publishing a new value, not by editing this file).
    // The subscriber vec offsets stay local: they are validated together with
    // the controller inside ResolveController and are not reachable from the
    // pattern repo today.
    ...
    void* ResolveController() {
        ...
        return *reinterpret_cast<void**>(
            static_cast<uint8_t*>(topMgr) + StructOffsets::ControllerInTopManager());
    }

    // Probe export consumed by StructOffsets::ValidateControllerOffset so the
    // offset resolver can structurally validate the controller slot against the
    // live top manager without a link-time dependency on this translation unit.
    // Returns nullptr until GetTopManager has been resolved. (The C symbol name
    // cannot start with a digit, hence the Oxo-prefixed export string.)
    extern "C" __declspec(dllexport) void* OxoProbeGetTopManager();
    void* OxoProbeGetTopManager() {
        return oGetTopManager ? oGetTopManager() : nullptr;
    }
    ...
    // in _0xoLemonCore's CSteamApp layout). g_pAppChangeSource capture moved to
    ...
    void RemoveAppOverview(AppId_t appId) {
        ...
        if (void* pApp = oGetAppByID(pController, appId, /*create=*/false)) {
            *reinterpret_cast<uint32_t*>(
                static_cast<uint8_t*>(pApp) + StructOffsets::CSteamAppOwnedFlagOffset()) &= ~1u;
        }
