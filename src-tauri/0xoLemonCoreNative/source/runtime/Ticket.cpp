...
namespace Ticket {
    ...
    uint64_t GetActiveSteamID64() {
        auto ReadCached = []() -> uint64_t {
            char buf[32] = {};
            DWORD sz = sizeof(buf);
            if (RegGetValueA(HKEY_CURRENT_USER, "Software\\Valve\\Steam\\_0xolemoncore",
        ...
        auto WriteCache = [](uint64_t sid) {
            ...
            HKEY hk = nullptr;
            if (RegCreateKeyExA(HKEY_CURRENT_USER, "Software\\Valve\\Steam\\_0xolemoncore",
    ...
    //  _0xoLemonCore. The list is not security-sensitive — it only changes
