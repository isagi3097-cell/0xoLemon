...
// through the runtime TOML. The result lands in <Steam>\_0xolemoncore\status.json
...
//   build_id            string
//   _0xolemoncore_build_stamp string (__DATE__ + __TIME__ from the DLL)
...
//   loader              string (proxy that loaded _0xoLemonCore)
//   hook_target         string (diversion or active_steamclient)
//   hook_module         string (path used for steamclient hook resolution)
//   struct_offsets      object { controller_in_top_manager, csteam_app_owned_flag,
//                                remote_storage_app_id, source, reason }
...
namespace HookStatus {
    ...
    // Publishes the resolved struct-offset table + the source it came from
    // ("default" / "config" / "config+validated") and the validation reason.
    ...
    // Writes the current snapshot to <Steam>\_0xolemoncore\status.json via a
