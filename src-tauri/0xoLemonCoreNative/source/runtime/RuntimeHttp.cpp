...
namespace {
    ...
    constexpr std::wstring_view kDefaultUserAgent = L"_0xoLemonCore-RuntimeHttp/1.0";
...
Response GetLimited(std::string_view url,
    ...
    DWORD redirectPolicy = 2; // WINHTTP_OPTION_REDIRECT_POLICY_ALWAYS
    WinHttpSetOption(req, 88 /* WINHTTP_OPTION_REDIRECT_POLICY */, &redirectPolicy, sizeof(redirectPolicy));

    DWORD decomp = 1 | 2; // GZIP | DEFLATE
    WinHttpSetOption(req, 118 /* WINHTTP_OPTION_DECOMPRESSION */, &decomp, sizeof(decomp));
    ...
    if (!WinHttpSendRequest(req,
                            ...
                            joinedHeaders.empty() ? 0 : (DWORD)-1L,
        ...
        || !WinHttpReceiveResponse(req, nullptr))
    {
        ...
        if (err == ERROR_WINHTTP_SECURE_FAILURE) {
            ...
            if (!WinHttpSendRequest(req,
                ...
                || !WinHttpReceiveResponse(req, nullptr))
            {
                r.diagnostic = "send/receive retry err=" + std::to_string(GetLastError());
                return r;
            }
        } else {
            r.diagnostic = "send/receive err=" + std::to_string(err);
            return r;
        }
    }
...
Response Post(std::string_view url, std::string_view body,
    ...
    WinHandle session(WinHttpOpen(L"_0xoLemonCore-RuntimeHttp/1.0",
                                  WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
                                  WINHTTP_NO_PROXY_NAME,
                                  WINHTTP_NO_PROXY_BYPASS, 0));
    if (!session) {
