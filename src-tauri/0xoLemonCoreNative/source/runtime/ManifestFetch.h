...
//
// The fix: when _0xoLemonCore sees an outgoing 151 with target_job_name
...
#ifndef OXOLEMONCORE_MANIFEST_FETCH_H
#define OXOLEMONCORE_MANIFEST_FETCH_H
...
namespace ManifestFetch {
    ...
    // Stops accepting new fetches and waits (bounded) for in-flight ones to
    // finish. Called from DLL_PROCESS_DETACH so a detached fetch thread can't
    // write freed globals during teardown. Idempotent.
    void Shutdown();
}
