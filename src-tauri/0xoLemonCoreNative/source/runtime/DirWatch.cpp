    ...
    static void MonitorThread()
    {
        ...
        while (g_alive) {
            ...
            if (!ordering.empty()) {
                ...
                HubcapManifestSync::TriggerSync();
                // A freshly dropped .lua may reference a game that already
                // finished downloading, so give the SteamStub watcher a chance
                // to strip it now instead of waiting for the next poll tick.
                SteamlessWatch::ScanNow();
