...
namespace HookStatus {
    ...
    namespace {
        ...
        std::uint64_t g_structOffController = 0;
        std::uint64_t g_structOffOwnedFlag = 0;
        std::uint64_t g_structOffRemoteStorage = 0;
        std::string g_structOffSource = "default";
        std::string g_structOffReason;
...
#ifdef OXOLEMONCORE_LOGGING_ENABLED
        ...
        constexpr const char* k0xoLemonCoreBuildStamp = __DATE__ " " __TIME__;
        constexpr const char* k0xoLemonCoreVersion = "V34";
        ...
        std::string SerializeLocked() {
            ...
            out += "\",\n";
            out += "  \"_0xolemoncore_build_stamp\": \"";
            out += JsonEscape(k0xoLemonCoreBuildStamp);
            out += "\",\n";
            out += "  \"_0xolemoncore_version\": \"";
            out += JsonEscape(k0xoLemonCoreVersion);
            out += "\",\n";
            out += "  \"build_config\": \"";
            ...
            out += ",\n";
            out += "  \"struct_offsets\": {\n";
            out += "    \"controller_in_top_manager\": ";
            out += std::to_string(g_structOffController);
            out += ",\n";
            out += "    \"csteam_app_owned_flag\": ";
            out += std::to_string(g_structOffOwnedFlag);
            out += ",\n";
            out += "    \"remote_storage_app_id\": ";
            out += std::to_string(g_structOffRemoteStorage);
            out += ",\n";
            out += "    \"source\": \"";
            out += JsonEscape(g_structOffSource);
            out += "\",\n";
            out += "    \"reason\": \"";
            out += JsonEscape(g_structOffReason);
            out += "\"\n";
            out += "  },\n";
        ...
        bool WriteBodyAtomic(const std::string& body) {
            ...
            std::filesystem::path dir = std::filesystem::path(SteamInstallPath) / "_0xolemoncore";
            std::error_code ec;
    ...
    void SetStructOffsetState(std::uint64_t controllerInTopManager,
                              std::uint64_t cSteamAppOwnedFlagOffset,
                              std::uint64_t remoteStorageAppIdOffset,
                              std::string source, std::string reason) {
        std::lock_guard<std::mutex> lk(g_mu);
        g_structOffController = controllerInTopManager;
        g_structOffOwnedFlag = cSteamAppOwnedFlagOffset;
        g_structOffRemoteStorage = remoteStorageAppIdOffset;
        g_structOffSource = std::move(source);
        g_structOffReason = std::move(reason);
        MaybeRepublishLocked();
    }
