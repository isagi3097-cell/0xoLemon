...
// _0xoLemonCore in-process layout descriptors.
//
// Each struct in this header pins the byte layout _0xoLemonCore needs in order
...
// field will fail the _0xoLemonCore build instead of corrupting state at run
// time.
...
struct CUtlVector {
	...
	// insertion order. _0xoLemonCore call sites that walk the vector once are
...
struct CUtlBuffer{
	...
	// Direct base + cursor accessors. _0xoLemonCore needs these so a hook
	...
	// Diagnostics dump, used by _0xoLemonCore logs to make a sliced buffer
...
// _0xoLemonCore reads these out of the resolved dependency vector and feeds
...
static_assert(sizeof(DepotEntry) == 0x20, "_0xoLemonCore depot record size drift: DepotEntry no longer matches the 0x20-byte layout _0xoLemonCore expects");
...
struct KeyValues
{
	...
	// +0x10 (4B), packed bitfield. Layout _0xoLemonCore depends on:
...
static_assert(sizeof(KeyValues) == 0x20, "_0xoLemonCore KeyValues node size drift: KeyValues no longer matches the 0x20-byte layout _0xoLemonCore expects");

// IKeyValuesSystem
//   _0xoLemonCore resolves this interface through the export
//   "KeyValuesSystemSteam" (ord 103) from vstdlib_s64.dll. _0xoLemonCore
...
struct IKeyValuesSystem {
	...
	// _0xoLemonCore does not call these, so the slots stay undeclared.

	// Convenience helpers used by _0xoLemonCore call sites.
...
// _0xoLemonCore branches on the high bit of eMsg to pick between the protobuf
...
// CSteamPipeClient: layout _0xoLemonCore reads back from the live pipe object.
