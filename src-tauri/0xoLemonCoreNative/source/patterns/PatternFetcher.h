...
// secondary and a local cache under <Steam>\_0xolemoncore\pattern\), parses it, and
...
namespace PatternFetcher {
    ...
    // network fetch. The local cache is flat at <Steam>\_0xolemoncore\pattern\
    ...
    // <Steam>\_0xolemoncore\pattern\<sha>.toml if present, installs the entries,
    ...
    // Drops all in-memory entry maps. Called from _0xoLemonCore::Detach.
    void Reset();
}
