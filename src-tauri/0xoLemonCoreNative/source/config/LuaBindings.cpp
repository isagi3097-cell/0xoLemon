...
namespace LuaLoader::Internal {
    ...
    namespace {
        // Hardcoded baseline. Anyone trying to run an official _0xoLemonCore
        ...
        constexpr std::string_view kHttpBaselineHosts[] = {
            ...
            "gitflic.ru",
            "api.github.com",
            // GitHub-independent mirror of the same manifests and metadata. Needed
            // because a flagged GitHub repository would otherwise take every
            // official workflow down with it.
            "huggingface.co",
        };
...
// ── Typed argument helpers ───────────────────────────────────────────
    ...
    // ── lcHttpGet(url [, headers]) -> body, status ───────────────────────────────────
    int Bind_lcHttpGet(lua_State* L) {
        ...
        const auto resp = RuntimeHttp::Get(url, extraHeaders);
        if (resp.networkError) {
    ...
    // ── steamlessApply(appId) -> status ────────────────────────────────
    // Strips SteamStub from the game's main executable on disk (the native
    // port of the launcher's Steamless button). BLOCKING — the caller (a
    // plugin .lua) runs on the DirWatch worker, not a Steam UI thread, so a
    // multi-second rewrite is acceptable. Returns a short status string.
    int Bind_steamlessApply(lua_State* L) {
        AppId_t appId = CheckAppId(L, 1, "steamlessApply");
        const std::string status = SteamlessWatch::FixAppNow(appId);
        lua_pushlstring(L, status.data(), status.size());
        return 1;
    }

    // ── steamlessRestore(appId) -> status ──────────────────────────────
    // Restores the pristine executable from the ".bak" backup. BLOCKING.
    int Bind_steamlessRestore(lua_State* L) {
        AppId_t appId = CheckAppId(L, 1, "steamlessRestore");
        const std::string status = SteamlessWatch::RestoreAppNow(appId);
        lua_pushlstring(L, status.data(), status.size());
        return 1;
    }
}
