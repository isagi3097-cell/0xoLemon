...
namespace LuaLoader {
    ...
    // True when the active Lua config explicitly asked for this depot to follow
    // the provider instead of a pinned gid — either `skipManifestPin(depotId)`
    // in the same file, or a depot whose only appearance is `addappid(...)`
    // (gid 0). Callers use this to decide whether a pinned- looking gid may be
    // replaced with the gid the provider currently serves.
    bool IsManifestAutoUpdate(uint64_t depotId);

    // Resolves parent AppId for a given depot or DLC AppId as loaded from .lua scripts.
    // Falls back to depotOrDlcId if no parent association is recorded.
