...
namespace CoreInit {
    ...
    namespace Diversion {
        ...
        // _0xoLemonCore copies it to bin\lcoverlay.dll and loads that copy. The SteamUI hook then
        ...
        bool PrepareAndLoad()
        {
            ...
            sprintf_s(ConfigPath,      MAX_PATH, "%s\\_0xolemoncore.toml",       SteamInstallPath);
            sprintf_s(PayloadPath,     MAX_PATH, "%s\\0xoPayload.dll", SteamInstallPath);
    ...
    namespace Bootstrap {
        ...
        DWORD Run(HMODULE selfModule)
        {
            ...
            // Point SteamlessApply at the launcher-shipped Steamless.CLI.exe +
            // its seven unpacker plugins. 0xoCore.dll drives that binary instead
            // of re-implementing SteamStub removal, so every variant atom0s
            // supports works here too. Set before SteamlessWatch::Start().
            Settings::ResolveSteamlessComponentDir();
            ...
            // Resolve client-object struct offsets now that the hook layer owns
            // the GetTopManager anchor. Reads _0xolemoncore.toml [struct_offsets]
            // and structurally validates the controller slot; falls back to the
            // built-in defaults when unresolved. Keeps ticket/library/remote-
            // storage paths working after a Valve layout shift without a code
            // change.
            StructOffsets::ResolveAtStartup();
            ...
            HubcapManifestSync::StartBackgroundWorker();
            HubcapManifestSync::StartAsyncWorker();
            AutoRetry::Start();
            // Strip SteamStub from Lua-tracked games once their download
            // finishes (opt-in via [steamstub] auto_steamless).
            SteamlessWatch::Start();
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD dwReason, PVOID pvReserved)
{
    if (dwReason == DLL_PROCESS_ATTACH)
    {
        ...
        // Pin the module so a stray FreeLibrary cannot unmap _0xoLemonCore while
    ...
    else if (dwReason == DLL_PROCESS_DETACH)
    {
        ...
        if (g_HooksInstalled.load()) {
            AutoRetry::Stop();
            SteamlessWatch::Stop();
            // Stop the async fetch worker first: it may be inside EnsureManifest
            // holding depotcache locks that the shutdown path would contend on.
            HubcapManifestSync::StopAsyncWorker();
            HubcapManifestSync::StopBackgroundWorker();
            // Drain detached HTTP workers that otherwise outlive the DLL and
            // touch freed anonymous-namespace globals during teardown — the
            // crash-on-exit class (stats/remote-API path). Both are bounded
            // waits so a hung endpoint can't stall the detach.
            StatsClient::Stop();
            ManifestFetch::Shutdown();
            DirWatch::Stop();
            if (pvReserved == nullptr) {
                SteamUI::CoreUnhook();
                _0xoLemonCore::Detach();
            }
        }
    }
